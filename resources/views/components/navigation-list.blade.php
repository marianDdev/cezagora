                ...
                @else
                    <a href="{{ route('checkout.show') }}"
                       class="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">
                        ...
                        <span
                            class="absolute right-0 top-0 rounded-full bg-red-600 w-4 h-4 top right p-0 m-0 text-white font-mono text-sm  leading-tight text-center">{{ \App\Models\Order::where('customer_id', \Illuminate\Support\Facades\Auth::user()->company->id)
                             ->where('status', Order::STATUS_PENDING)
                             ->first()
                             ->items
                             ->count() }}
